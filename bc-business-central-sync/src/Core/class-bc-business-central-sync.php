...
class BC_Business_Central_Sync {
	...
	private function load_integration_classes() {
		...
		require_once BC_BUSINESS_CENTRAL_SYNC_PATH . 'includes/class-bc-simple-pricing.php';
	}
	 ...
	 * @access private
	 */
	private function init_database_migration() {
		// Check if database needs migration
		if ( class_exists( 'BC_Database_Migration' ) ) {
			BC_Database_Migration::maybe_migrate();
		}
	}

	/**
	 * Initialize WooCommerce pricing integration.
	 *
	 * @since 1.0.0
	 */
	public function init_woocommerce_pricing() {
		// Only initialize if WooCommerce is active
		if ( class_exists( 'WooCommerce' ) ) {
			// Initialize simple pricing system (no HPOS complexity)
			if ( class_exists( 'BC_Simple_Pricing' ) ) {
				new BC_Simple_Pricing();
			}
			
			// Fallback to traditional pricing if needed
			if ( class_exists( 'BC_WooCommerce_Pricing' ) ) {
				new BC_WooCommerce_Pricing();
			}
		}
	}
	...
	private function define_public_hooks() {
		...
		// Initialize WooCommerce pricing integration
		$this->loader->add_action( 'init', $this, 'init_woocommerce_pricing' );
	}
